...
export default function ProductsVariantList() {
  ...
  useEffect(() => {
    function loadProduct() {
      const productVariant = location.state.params;
  ...
  const handleNavigateEditProductVariant = async (variant) => {
    try {
      ...
      history.push('/produtos/editar', { params: product, productScreen: false, variantId: variant.id });
    } catch(err) {
      alert(
  ...
  const handleNavigateDeleteProduct = async (variant) => {
    try {
      const response = await api.delete(`/skus/${variant.id}`);
      if (response.data.id) {
        alert('A variante do produto foi excluída com sucesso');

        const responseProduct = await api.get(`/products/${variant.product_id}`);
        setProduct(responseProduct.data);
      }
    } catch (err) {
      alert(
        'Ocorreu um erro ao excluir a variante do produto ' +
          JSON.stringify(err)
      );
    }
  };
          ...
          <Grid container spacing={3}>
            <Grid item sm={12}>
              <Paper className={classes.paper}>
                <h2>Variantes do Produto - ID {product.id}</h2>
                <Grid container spacing={3}>
                  <Grid item sm={12}>
                        ...
                        <TableBody>
                          {product.variants.map((variant) => (
                            <TableRow>
                              ...
                              <TableCell align="center">
                                {variant.height.toFixed(2)} Centímetros
                              </TableCell>
                              <TableCell align="center">
                                {variant.weight.toFixed(2)} Gramas
                              </TableCell>
                              <TableCell align="center">
                                {variant.length.toFixed(2)} Centímetros
                              </TableCell>
                              <TableCell align="center">
                                {variant.width.toFixed(2)} Centímetros
                              </TableCell>
                              <TableCell align="center">
                              ...
                              <TableCell align="center">
                                <Button
                                  ...
                                  onClick={() => {
                                    console.log(variant);
                                    setVariantSelectedImages(variant.images);
                              ...
                              <TableCell align="center">
                                <Button
                                  ...
                                  onClick={() =>
                                    handleNavigateEditProductVariant(variant)
                                  }
                                >
                                  Editar
                                </Button>
                                <Button
                                  ...
                                  style={{ width: '90%' }}
                                  onClick={() =>
                                    handleNavigateDeleteProduct(variant)
                                  }
                                >
                                  Deletar
                                </Button>
